...
*/
NN_DIGIT tMult[2*MAX_NN_DIGITS];
    ...
    for (i = 0; i < bDigits; i++) {
        ...
        if(*(b+i) != 0) {
            for(j = 0; j < cDigits; j++) {
                ...
                if((*(tMult+(i+j)) = *(tMult+(i+j)) + carry) < carry)
                    carry = 1;
                else
                    carry = 0;
                if((*(tMult+(i+j)) += dlow) < dlow)
                    carry++;
                carry += dhigh;
            }
        }
        *(tMult+(i+cDigits)) += carry;
    }


    NN_Assign(a, tMult, 2 * digits);
}
...
*/
NN_DIGIT temp[2*MAX_NN_DIGITS];

void NN_ModMult (a, b, c, d, digits)
NN_DIGIT *a, *b, *c, *d;
unsigned int digits;
{
    NN_Mult (temp, b, c, digits);
    NN_Mod (a, temp, 2 * digits, d, digits);
}
...
NN_DIGIT tModExp[MAX_NN_DIGITS];
...
unsigned int cDigits, dDigits;
{
    ...
    */
    ...
    NN_ASSIGN_DIGIT (tModExp, 1, dDigits);
    ...
    for (i = cDigits - 1; i >= 0; i--) {
        ...
        */
        ...
        for (j = 0; j < ciBits; j += 2, ci <<= 2) {
            ...
            NN_ModMult (tModExp, tModExp, tModExp, d, dDigits);
            NN_ModMult (tModExp, tModExp, tModExp, d, dDigits);
            if ((s = DIGIT_2MSB (ci)) != 0)
                NN_ModMult (tModExp, tModExp, bPower[s-1], d, dDigits);
        }
    }

    NN_Assign (a, tModExp, dDigits);
}
